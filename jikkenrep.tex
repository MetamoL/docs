...
\begin{table}[htb]
\begin{tabular}{|l|l|l|l|l|l|l|} \hline
 最大遅延(ns) & 最大遅延(比) & 最大動作周波数(MHz) & 等価ゲート数　& 面積(1000m^{2}) & 消費電力(W/MHz) & 最大消費電力(mW) \\ \hline
...
32bit幅x32のレジスタ.8bit幅x32のレジスタ4個から成る.5bitのアドレス入力信号によって指定されるレジスタに32bit幅の入力信号を書き込む.あるいは指定のレジスタの内容を32bit信号で出力する.
\begin{table}[htb]
\begin{tabular}{|l|l|l|l|l|l|l|} \hline
最大遅延(ns) & 最大遅延(比) & 最大動作周波数(MHz) & 等価ゲート数　& 面積(1000{\mu}m^{2}) & 消費電力({\mu}W/MHz) & 最大消費電力(mW) \\ \hline
